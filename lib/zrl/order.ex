defmodule Zrl.Order do
  ...
  alias Zrl.Order.Movement
  alias Zrl.MovementExceptions.MovementException
  ...
  def search_for_train_list_entry(wagon_id, train_no) do
    ...
    |> join(:left, [m], c in Zrl.Order.Consignment,
      on:
  ...
  alias Zrl.Order.FuelMonitoring
  ...
  def get_fuel_monitor_by_date(quarter, year) do
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.Refueling, on: a.refuel_type == b.id)
    |> join(:left, [a, b], c in Zrl.SystemUtilities.Rates, on: a.depo_refueled_id == c.id)
  ...
  def lookup_weekly_tonnage(month, year) do
    ...
    |> join(:left, [a], b in Zrl.Order.Movement, on: a.train_number == b.train_no)
    |> join(:left, [a, b], c in Zrl.Order.Consignment, on: c.document_date == b.consignment_date)
    |> where(
      [a, b],
      exists(
        from(m in Zrl.Order.Movement, where: parent_as(:requisite).train_number == m.train_no)
  ...
  def get_consumption_by_routes(month, year) do
    ...
    |> join(:left, [a], b in Zrl.Order.Movement, on: a.train_number == b.train_no)
    |> join(:left, [a, b], c in Zrl.Order.Consignment, on: c.document_date == b.consignment_date)
    |> join(:left, [a, b, c], d in Zrl.SystemUtilities.Section, on: a.section_id == d.id)
    |> where(
      [a, b],
      exists(
        from(m in Zrl.Order.Movement, where: parent_as(:requisite).train_number == m.train_no)
  ...
  def get_users_name() do
    FuelMonitoring
    |> join(:left, [a], b in Zrl.Accounts.User, on: a.commercial_clerk_id == b.id)
    |> select([a, b], %{
  ...
  def data_batch(status) do
    Batch
    |> join(:left, [a], b in Zrl.Order.Consignment, on: a.id == b.batch_id)
  ...
  def data_entry_batch(user_id) do
    Batch
    |> join(:left, [a], b in Zrl.Order.Consignment, on: a.id == b.batch_id)
    |> join(:left, [a, _b], c in Zrl.Accounts.User, on: a.last_user_id == c.id)
  ...
  def list_batch_items(batch_id, empty_commodity) do
    ...
    |> join(:left, [a, b, _c, _d], e in Zrl.SystemUtilities.TariffLine, on: e.id == a.tarrif_id)
    |> join(:left, [a, b, _c, _d, e], f in Zrl.SystemUtilities.Surchage,
  ...
  def monthly_income_summary(start_end, end_date, unmatched_period, user) do
    from(a in Zrl.Order.Consignment, as: :consign)
    |> where(
      [a],
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.Commodity, on: a.commodity_id == b.id)
    |> join(:left, [a, b], c in Zrl.SystemUtilities.TariffLine, on: a.tarrif_id == c.id)
    |> join(:left, [a, b, c], d in Zrl.SystemUtilities.Currency, on: c.currency_id == d.id)
    |> join(:left, [a, b, c, d], e in Zrl.SystemUtilities.TrainRoute, on: a.route_id == e.id)
    |> join(:left, [a, b, c, d, e], f in Zrl.SystemUtilities.TransportType,
      on: e.transport_type == f.id
    )
    |> join(:left, [a, b, c, d, e, f], g in Zrl.Order.Movement,
      on:
  ...
  def unmatched_unaging(start_end, end_date, unmatched_period, user) do
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.Commodity, on: a.commodity_id == b.id)
    |> join(:left, [a, b], c in Zrl.SystemUtilities.TariffLine, on: a.tarrif_id == c.id)
    |> join(:left, [a, b, c, d], d in Zrl.SystemUtilities.Currency, on: c.currency_id == d.id)
    |> join(:left, [a, b, c, d], e in Zrl.SystemUtilities.TrainRoute, on: a.route_id == e.id)
    |> join(:left, [a, b, c, d, e], f in Zrl.SystemUtilities.TransportType,
      on: e.transport_type == f.id
    )
    # |> join(:left, [a, b, c, e, f, ], g in Zrl.Order.Movement, on: a.commodity_id == g.commodity_id and a.document_date == g.consignment_date and a.final_destination_id == g.destin_station_id and a.origin_station_id == g.origin_station_id and a.wagon_id == g.wagon_id and a.consignee_id == g.consignee_id and a.consigner_id == g.consigner_id)
    |> where(
      [a, b, c, d, e, f],
      exists(
        from(m in Zrl.Order.Movement,
  ...
  def movement_report_lookup(search_params, page, size, user) do
    ...
    |> join(:left, [a, b], c in Zrl.SystemUtilities.WagonType, on: b.wagon_type_id == c.id)
    |> join(:left, [a, b, _c], d in Zrl.Accounts.RailwayAdministrator, on: b.owner_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.Accounts.Clients, on: a.consigner_id == e.id)
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.Clients, on: a.consignee_id == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.Accounts.Clients, on: a.payer_id == g.id)
    |> join(:left, [a, _b, _c, _d, _e, _f, _g], h in Zrl.SystemUtilities.Station,
      on: a.movement_destination_id == h.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h], i in Zrl.SystemUtilities.Station,
      on: a.movement_origin_id == i.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i], j in Zrl.SystemUtilities.Commodity,
      on: a.commodity_id == j.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i, j], k in Zrl.SystemUtilities.Station,
      on: a.movement_reporting_station_id == k.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k], z in Zrl.Accounts.Clients,
  ...
  def movement_report_lookup_excel(_source, search_params, settings, user) do
    ...
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i, j, k, _z], l in Zrl.Order.Consignment,
      on:
  ...
  def consignment_report_lookup(search_params, page, size, user, empty_commodity) do
    ...
    |> join(:left, [a, b], c in Zrl.SystemUtilities.WagonType, on: b.wagon_type_id == c.id)
    |> join(:left, [a, b, _c], d in Zrl.Accounts.RailwayAdministrator, on: b.owner_id == d.id)
    |> join(:left, [a, b, _c, _d], e in Zrl.SystemUtilities.Station,
      on: a.tariff_destination_id == e.id
    )
    |> join(:left, [a, b, _c, _d, _e], f in Zrl.SystemUtilities.Station,
      on: a.tariff_origin_id == f.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f], g in Zrl.SystemUtilities.Station,
      on: a.final_destination_id == g.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g], h in Zrl.SystemUtilities.Station,
      on: a.origin_station_id == h.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h], i in Zrl.SystemUtilities.Station,
      on: a.reporting_station_id == i.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i], j in Zrl.SystemUtilities.Commodity,
      on: a.commodity_id == j.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _j], k in Zrl.Accounts.Clients,
      on: a.consignee_id == k.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _j, _k], l in Zrl.Accounts.Clients,
      on: a.consigner_id == l.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l], m in Zrl.Accounts.Clients,
      on: a.customer_id == m.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m], n in Zrl.Accounts.Clients,
      on: a.payer_id == n.id
    )
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n], o in Zrl.Order.Batch,
      on: a.batch_id == o.id
    )
  ...
  def consignment_report_excel(_source, search_params, settings, user) do
    ...
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, n], o in Zrl.Order.Movement,
      on:
    ...
    |> join(
      ...
      p in Zrl.SystemUtilities.Currency,
  ...
  defp compose_consignment_report_select(query, empty_commodity) do
    ...
    |> where(
      [a, b, c, d, e, f, g, h, i, j, k, l, m, n],
      a.id in subquery(
        from(t in Zrl.Order.Consignment,
          where:
  ...
  def tarrif_rates(tarrif_id) do
    Zrl.SystemUtilities.TariffLineRate
  ...
  def list_consignment_batch_item(batch_id) do
    ...
    |> join(
      ...
      o in Zrl.SystemUtilities.TariffLine,
      on: o.id == a.tarrif_id
    )
    |> join(
      :left,
      [a, b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m, _n, o],
      p in Zrl.SystemUtilities.Surchage,
  ...
  def consignment_batch_lookup_excel(status, user) do
    unmatched_aging = Zrl.SystemUtilities.list_company_info().unmatched_aging_period
  ...
  def fuel_report_lookup(search_params, page, size, user) do
    Zrl.Order.FuelMonitoring
    |> where([a], a.status == "COMPLETE")
    # |> verify_user_region_id(user)
    |> verify_depo_requisites(user)
    |> join(:left, [a], b in Zrl.SystemUtilities.Rates, on: a.depo_refueled_id == b.id)
    |> join(:left, [a, b], c in Zrl.SystemUtilities.Station, on: a.train_destination_id == c.id)
    |> join(:left, [a, b, _c], d in Zrl.Accounts.User, on: a.commercial_clerk_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.SystemUtilities.TrainType,
      on: a.train_type_id == e.id
    )
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.User, on: a.locomotive_driver_id == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.Locomotives.LocomotiveType,
      on: a.loco_id == g.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g], h in Zrl.SystemUtilities.Station,
      on: a.train_origin_id == h.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h], i in Zrl.SystemUtilities.Refueling,
      on: a.refuel_type == i.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i], j in Zrl.SystemUtilities.Section,
      on: a.section_id == j.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i, _j], k in Zrl.Locomotives.Locomotive,
  ...
  defp compose_fuel_report_select(query) do
    ...
    |> where(
      [a, b, c, d, e, f, g, h, i, j, k, l, m, n],
      a.id in subquery(
        from(t in Zrl.Order.FuelMonitoring,
  ...
  def list_movement_batch_items(batch_id) do
    Zrl.Order.Movement
    |> where([a], a.batch_id == ^batch_id)
    |> join(:left, [a], b in Zrl.SystemUtilities.Station,
  ...
  def movement_draft_batches(user) do
    Batch
    |> join(:left, [a], b in Zrl.Order.Movement, on: a.id == b.batch_id)
    |> join(:left, [a, _b], c in Zrl.Accounts.User, on: a.last_user_id == c.id)
  ...
  def all_movement_batch_entries(batch_id, status, unmatched_aging) do
    ...
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i, j], l in Zrl.Order.Consignment,
      on:
        l.status in ["COMPLETE", "PENDING_INVOICE"] and l.commodity_id == a.commodity_id and
          l.document_date == a.consignment_date and l.final_destination_id == a.destin_station_id and
          l.origin_station_id == a.origin_station_id and l.wagon_id == a.wagon_id and
  ...
  def mvt_lookup_train_no(train_no) do
    ...
    |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _l], n in Zrl.SystemUtilities.Station,
      on: b.station_id == n.id
    )
    # |> join(:left, [a, b, _c, _d, _e, _f, _g, _h, _i, _l, _n], o in Zrl.SystemUtilities.Condition, on: b.condition_id == o.id)
  ...
  def movement_report_entry_lookup(id) do
    ...
    |> join(:left, [a, b], c in Zrl.SystemUtilities.WagonType, on: b.wagon_type_id == c.id)
    |> join(:left, [a, b, _c], d in Zrl.Accounts.RailwayAdministrator, on: b.owner_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.Accounts.Clients, on: a.consigner == e.id)
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.Clients, on: a.consignee == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.Accounts.Clients, on: a.payer_id == g.id)
    |> join(:left, [a, _b, _c, _d, _e, _f, _g], h in Zrl.SystemUtilities.Station,
      on: a.movement_destination_id == h.id
    )
    ...
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i, _j], k in Zrl.Order.Consignment,
      on:
  ...
  def user_type_name(%{batch_id: batch_id}) do
    from(a in Zrl.Order.FuelMonitoring, where: a.batch_id == ^batch_id)
    |> select(
      [a],
      map(
        a,
  ...
  def intransit_train_lookup(train_no) do
    ...
    |> join(:left, [a, _b], c in Zrl.SystemUtilities.Station,
  ...
  def all_fuel_pending_monitoring(id, status, user) do
    ...
    |> join(:left, [a, b], c in Zrl.SystemUtilities.Station, on: a.train_destination_id == c.id)
    |> join(:left, [a, b, _c], d in Zrl.Accounts.User, on: a.commercial_clerk_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.SystemUtilities.TrainType, on: a.train_type_id == e.id)
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.User, on: a.locomotive_driver_id == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.Locomotives.LocomotiveType, on: a.loco_id == g.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.SystemUtilities.TrainType,
      on: a.train_type_id == e.id
    )
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.LocoDriver, on: a.loco_driver_id == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.Locomotives.LocomotiveType,
      on: a.loco_id == g.id
    )
    |> select([a, b, c, d, e, f, g], %{
  ...
  def works_order_report_lookup(search_params, page, size, _user) do
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.Wagon, on: a.wagon_id == b.id)
    |> join(:left, [a, b], c in Zrl.Accounts.RailwayAdministrator, on: b.owner_id == c.id)
    |> join(:left, [a, b, c], d in Zrl.Accounts.Clients, on: a.client_id == d.id)
    |> join(:left, [a, b, c, d], e in Zrl.SystemUtilities.Commodity, on: a.commodity_id == e.id)
