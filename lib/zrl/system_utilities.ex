defmodule Zrl.SystemUtilities do
  @moduledoc """
  The SystemUtilities context.
  """
  # Zrl.SystemUtilities.filter_distance_lookup(nil, 1, 10, nil)
  ...
  def tariffline_lookup(client_id, orig_station_id, destin_station_id, commodity, date) do
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.TariffLineRate, on: a.id == b.tariff_id)
    |> join(:left, [a, b], c in Zrl.Accounts.RailwayAdministrator, on: b.admin_id == c.id)
    |> join(:left, [a, b, c], d in Zrl.SystemUtilities.Country, on: c.country_id == d.id)
    |> join(:left, [a, b, c, d], e in Zrl.SystemUtilities.Surchage, on: a.surcharge_id == e.id)
    |> where(
      [a, b, c, d, e],
      a.id in subquery(
        from(
  ...
  alias Zrl.SystemUtilities.Commodity
  ...
  alias Zrl.SystemUtilities.CommodityGroup
  ...
  def wagon_lookup(code) do
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.WagonType, on: a.wagon_type_id == b.id)
    |> join(:left, [a, b], c in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == c.id)
    |> select([a, b, c], %{
      id: a.id,
      wagon_owner: c.code,
      wagon_code: a.code,
  ...
  def wagon_fleet_lookup(search_params, page, size, _user) do
    Wagon
    |> join(:left, [a], b in Zrl.SystemUtilities.WagonType, on: a.wagon_type_id == b.id)
    |> join(:left, [a, _b], c in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == c.id)
    |> join(:left, [a, _b, _c], d in Zrl.Accounts.Clients, on: a.allocated_cust_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.Accounts.User, on: a.maker_id == e.id)
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.User, on: a.checker_id == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.SystemUtilities.Station,
      on: a.station_id == g.id
    )
    |> join(:left, [a, _b, _c, _d, _e, _f, _g], h in Zrl.SystemUtilities.Condition,
      on: a.condition_id == h.id
    )
    |> join(:left,  [a, _b, _c, _d, _e, _f, _g, _h], i in Zrl.SystemUtilities.WagonType, on: a.wagon_sub_type_id == i.id)
  ...
  def zrl_wagon_lookup_by_symbol(owner_id) do
    Wagon
  ...
  def zrl_wagon_lookup_by_domain(owner_id) do
    Wagon
    |> where([a], a.owner_id == ^owner_id and a.status == "A")
    |> join(:left, [a], b in Zrl.SystemUtilities.WagonType, on: a.wagon_type_id == b.id)
    |> join(:left, [a, _b], c in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == c.id)
    ...
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h], l in Zrl.SystemUtilities.Domain,
      on: a.domain_id == l.id
    )
  ...
  def all_zrl_wagon_lookup_by_symbol(owner_id) do
    Wagon
    |> where([a], a.owner_id == ^owner_id and a.status == "A")
    |> join(:left, [a], b in Zrl.SystemUtilities.WagonType, on: a.wagon_type_id == b.id)
    |> join(:left, [a, _b], c in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == c.id)
  ...
  def zrl_go_wagon_lookup_by_symbol(owner_id) do
    Wagon
    |> where([a], a.owner_id == ^owner_id and a.status == "A")
    |> join(:left, [a], b in Zrl.SystemUtilities.WagonType, on: a.wagon_type_id == b.id)
    |> join(:left, [a, _b], c in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == c.id)
  ...
  def zrl_lookup_by_load_status(owner_id) do
    Wagon
    |> where([a], a.owner_id == ^owner_id and a.status == "A")
    |> join(:left, [a], b in Zrl.SystemUtilities.WagonType, on: a.wagon_type_id == b.id)
    |> join(:left, [a, _b], c in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == c.id)
  ...
  alias Zrl.SystemUtilities.WagonType
  ...
  alias Zrl.SystemUtilities.Surchage
  ...
  alias Zrl.SystemUtilities.Station
  ...
  def station_owner_lookup(movement_reporting_station_id) do
    ...
    |> join(:left, [a], b in Zrl.Accounts.RailwayAdministrator, on: a.owner_id == b.id)
  ...
  alias Zrl.SystemUtilities.PaymentType
  ...
  alias Zrl.SystemUtilities.Country
  ...
  alias Zrl.SystemUtilities.Currency
  ...
  alias Zrl.SystemUtilities.Rates
  ...
  alias Zrl.SystemUtilities.ExchangeRate
  ...
  def get_weekly_fuel_request(month, year) do
    Rates
    |> join(:left, [a], b in Zrl.Order.FuelMonitoring,
  ...
  def get_fuel_rate_by_date(quarter, year) do
    Rates
    |> join(:left, [a], b in Zrl.Order.FuelMonitoring, on: a.id == b.depo_refueled_id)
    |> distinct(true)
    |> where(
      [a, b],
  ...
  def get_depo_summary(start_end, end_date, user) do
    ...
    |> join(:left, [a, b], c in Zrl.SystemUtilities.Station, on: a.depo_stn == c.id)
  ...
  def get_by_section(start_end, end_date, user) do
    ...
    |> verify_user_region_id(user)
    |> join(:left, [a], b in Zrl.SystemUtilities.Rates, on: a.depo_refueled_id == b.id)
    |> join(:left, [a, b], c in Zrl.SystemUtilities.Section, on: a.section_id == c.id)
  ...
  alias Zrl.SystemUtilities.TrainRoute
  ...
  def train_route_lookup(search_params, page, size, _user) do
    TrainRoute
    |> join(:left, [a], b in Zrl.SystemUtilities.Station, on: a.destination_station == b.id)
    |> join(:left, [a, _b], c in Zrl.SystemUtilities.Station, on: a.origin_station == c.id)
    |> join(:left, [a, _b, _c], d in Zrl.SystemUtilities.TransportType,
      on: a.transport_type == d.id
    )
    |> join(:left, [a, _b, _c, _d], e in Zrl.Accounts.RailwayAdministrator, on: a.operator == e.id)
    |> join(:left, [a, _b, _c, _d, _e], f in Zrl.Accounts.User, on: a.maker_id == f.id)
    |> join(:left, [a, _b, _c, _d, _e, _f], g in Zrl.Accounts.User, on: a.checker_id == g.id)
  ...
  alias Zrl.SystemUtilities.Model
  ...
  alias Zrl.SystemUtilities.Status
  ...
  def wagon_status_lookup(code) do
    Zrl.SystemUtilities.Status
    |> where([a], fragment("lower(?) = ?", a.code, ^remove_spaces(code)))
    |> Repo.one()
  end

  alias Zrl.SystemUtilities.TransportType
  ...
  alias Zrl.SystemUtilities.Condition
  ...
  alias Zrl.SystemUtilities.Spare
  ...
  alias Zrl.SystemUtilities.SpareFee
  ...
  alias Zrl.SystemUtilities.Defect
  ...
  def defect_spare_lookup(id, admin, date) do
    Zrl.SystemUtilities.DefectSpare
    # |> join(:left, [a], b in Zrl.SystemUtilities.DefectSpare, on: b.defect_id == a.id)
    |> where([b], b.defect_id == ^id)
    |> join(:left, [b], c in Zrl.SystemUtilities.Spare, on: b.spare_id == c.id)
    |> join(:left, [b, c], d in Zrl.SystemUtilities.SpareFee,
      on: d.spare_id == c.id and d.railway_admin == ^admin
    )
    |> join(:left, [b, c, d], e in Zrl.SystemUtilities.Currency, on: e.id == d.currency_id)
    |> where(
      [b, c, d, e],
      d.status == "A" and fragment("CAST(? AS DATE) >= ?", ^date, d.start_date)
    )
    # |> where([ b, c, d, e], d.id in subquery(from(f in Zrl.SystemUtilities.SpareFee, where: f.railway_admin == ^admin, select: f.id)))
  ...
  def interchange_defect_spare_lookup(id, admin) do
    ...
    |> join(:left, [a], b in Zrl.SystemUtilities.DefectSpare, on: a.defect_id == b.defect_id)
    |> join(:left, [a, b], c in Zrl.SystemUtilities.Spare, on: b.spare_id == c.id)
    |> join(:left, [a, b, c], d in Zrl.SystemUtilities.SpareFee,
      on: d.spare_id == c.id and d.railway_admin == ^admin
    )
    |> join(:left, [a, b, c, d], e in Zrl.SystemUtilities.Currency, on: e.id == d.currency_id)
    |> join(:left, [a, b, c, d, e], f in Zrl.SystemUtilities.Defect, on: f.id == a.defect_id)
  ...
  alias Zrl.SystemUtilities.InterchangeFee
  ...
  alias Zrl.SystemUtilities.TrainType
  ...
  alias Zrl.SystemUtilities.Domain
  ...
  def domain_lookup(code) do
    Zrl.SystemUtilities.Domain
    # |> where([a],  a.code == ^code )
    |> where([a], fragment("lower(?) = ?", a.code, ^remove_spaces(code)) and a.status == "A")
    |> Repo.one()
  end

  alias Zrl.SystemUtilities.CompanyInfo
  ...
  alias Zrl.SystemUtilities.Distance
  ...
  def filter_distance_lookup(search_params, page, size, _user) do
    Distance
    |> join(:left, [a], b in Zrl.SystemUtilities.Station, on: a.destin == b.id)
    |> join(:left, [a, _b], c in Zrl.SystemUtilities.Station, on: a.station_orig == c.id)
    |> join(:left, [a, _b, _c], d in Zrl.Accounts.User, on: a.maker_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.Accounts.User, on: a.checker_id == e.id)
  ...
  alias Zrl.SystemUtilities.TariffLineRate
  ...
  def tariff_line_rates_lookup(search_params, page, size, _user) do
    ...
    |> join(:left, [a, _b], c in Zrl.SystemUtilities.Currency, on: a.currency_id == c.id)
    |> join(:left, [a, _b, _c], d in Zrl.SystemUtilities.PaymentType, on: a.pay_type_id == d.id)
    |> join(:left, [a, _b, _c, _d], e in Zrl.SystemUtilities.Surchage, on: a.surcharge_id == e.id)
    ...
    |> join(:left, [a, _b, _c, _d, _e, _f, _g], h in Zrl.Accounts.Clients, on: a.client_id == h.id)
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h], i in Zrl.Accounts.User, on: a.maker_id == i.id)
    |> join(:left, [a, _b, _c, _d, _e, _f, _g, _h, _i], j in Zrl.Accounts.User,
  ...
  alias Zrl.SystemUtilities.Refueling
  ...
  alias Zrl.SystemUtilities.Section
  ...
  alias Zrl.SystemUtilities.Wagon_defect
  ...
  def tracker_entry_lookup(tracker_id, wagon_id) do
    Zrl.SystemUtilities.Wagon_defect
    |> where([a], a.tracker_id == ^tracker_id and a.wagon_id == ^wagon_id)
    |> limit(1)
    |> Repo.one()
  end

  alias Zrl.SystemUtilities.ConditionCategory
  ...
  alias Zrl.SystemUtilities.FileUploadError
  ...
  def exceptions_lookup(%{"type" => type} = search_params, page, size, _user) do
    FileUploadError
    |> where([a], a.type == ^type)
    |> join(:left, [a], b in Zrl.Accounts.User, on: a.user_id == b.id)
  ...
  alias Zrl.SystemUtilities.DefectSpare
  ...
  alias Zrl.SystemUtilities.CollectionType
  ...
  alias Zrl.SystemUtilities.Equipment
  ...
  alias Zrl.SystemUtilities.EquipmentRate
  ...
  alias Zrl.SystemUtilities.LocoDetentionRate
  ...
  alias Zrl.SystemUtilities.HaulageRate
