defmodule Zrl.Tracking.Auxiliary do
  use Ecto.Schema
  use Endon
  ...
  schema "tbl_interchange_auxiliary" do
    ...
    belongs_to :current_wagon, Zrl.SystemUtilities.Wagon,
    ...
    belongs_to :interchange_point, Zrl.SystemUtilities.Station,
      foreign_key: :interchange_point_id,
      type: :id

    belongs_to :admin, Zrl.Accounts.RailwayAdministrator, foreign_key: :admin_id, type: :id
    belongs_to :equipment, Zrl.SystemUtilities.Equipment, foreign_key: :equipment_id, type: :id
    ...
    belongs_to :equipment_rate, Zrl.SystemUtilities.EquipmentRate,
      foreign_key: :equipment_rate_id,
      type: :id

    belongs_to :hire_off_user, Zrl.Accounts.User, foreign_key: :hire_off_user_id, type: :id
    belongs_to :archive_user, Zrl.Accounts.User, foreign_key: :archive_user_id, type: :id
  ...
  defp does_auxiliary_fee_exist?(%Ecto.Changeset{valid?: true} = changeset) do
    ...
    case Zrl.SystemUtilities.material_fee_lookup(date, admin, equipment) do
      nil ->
        add_error(
          changeset,
          :Rate,
 ...
 defp find_wagon_by_code( %Ecto.Changeset{valid?: true, changes: %{wagon_code: wagon_code}} = changeset) do
    ...
    case Zrl.SystemUtilities.wagon_lookup(String.trim(wagon_code)) do
      nil ->
