defmodule Zrl.Workers.InterchangeAccumulativeDays do
  alias Zrl.Tracking
  alias Zrl.Tracking.Interchange
  alias Zrl.{SystemUtilities, Emails.Email, Notifications}
  alias ZrlWeb.InterchangeController
  alias Zrl.Tracking.{Auxiliary, WagonTracking}
  alias Zrl.Repo
  require Logger
  ...
  def update_wagon_days_at_station() do
    items = Zrl.Tracking.list_wagon_tracker_with_wagon_id()

    Enum.with_index(items, 1)
    |> Enum.map(fn {item, index} ->
      entry = Zrl.Tracking.get_wagon_tracking!(item)
