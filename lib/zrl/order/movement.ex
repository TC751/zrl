defmodule Zrl.Order.Movement do
  use Ecto.Schema
  ...
  schema "tbl_movement" do
    ...
    belongs_to :destin_station, Zrl.SystemUtilities.Station,
      foreign_key: :destin_station_id,
      type: :id

    belongs_to :commodity, Zrl.SystemUtilities.Commodity, foreign_key: :commodity_id, type: :id
    belongs_to :loco, Zrl.Locomotives.Locomotive, foreign_key: :loco_id, type: :id
    ...
    belongs_to :consignment, Zrl.Order.Consignment, foreign_key: :consignment_id, type: :id
    belongs_to :batch, Zrl.Order.Batch, foreign_key: :batch_id, type: :id

    belongs_to :movement_destination, Zrl.SystemUtilities.Station,
      foreign_key: :movement_destination_id,
      type: :id

    belongs_to :movement_reporting_station, Zrl.SystemUtilities.Station,
      foreign_key: :movement_reporting_station_id,
      type: :id

    belongs_to :movement_origin, Zrl.SystemUtilities.Station,
  ...
  defp does_consignment_exist?(%Ecto.Changeset{valid?: true} = changeset) do
    ...
    case invoice_no do
      ...
      _ ->
        case Zrl.Order.search_for_consignment_by_station_code(wagon_id, String.trim(invoice_no)) do
          nil ->
            wagon = Zrl.SystemUtilities.get_wagon!(wagon_id).code
  ...
  defp search_for_consignment(%Ecto.Changeset{valid?: true} = changeset) do
    ...
    consignee_id = get_field(changeset, :consignee_id)
      ...
      case consignee_id do
        ...
        _ ->
          case Zrl.Order.search_for_consignment(wagon_id, commodity_id, consignment_date, origin_station_id, destin_station_id, consignee_id) do
            nil ->
              changeset
