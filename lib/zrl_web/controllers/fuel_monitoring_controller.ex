defmodule ZrlWeb.FuelMonitoringController do
  use ZrlWeb, :controller
  use Timex

  alias Zrl.Order.FuelMonitoring
  alias Zrl.Activity.UserLog
  alias Zrl.Repo
  alias Zrl.SystemUtilities
  alias Zrl.Accounts
  alias Zrl.Locomotives
  alias Zrl.Accounts
  alias Zrl.Order
  alias ZrlWeb.InterchangeController

  @current "tbl_fuel_monitoring"

  plug(
    ZrlWeb.Plugs.RequireAuth
    when action not in [:unknown]
  )

  plug(
    ZrlWeb.Plugs.EnforcePasswordPolicy
    when action not in [:unknown]
  )

  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.FuelMonitoringController.authorize/1]
  ...
  def fuel_order(conn, _params) do
    ...
    case Zrl.Order.create_batch(params) do
      {:ok, batch} ->
        batch = Zrl.Order.get_by_uuid(batch.uuid)
      ...
      {:error, changeset} ->
        reason = ZrlWeb.UserController.traverse_errors(changeset.errors)
  ...
  def control_fuel_verification(conn, params) do
    ...
    fuel_requisite = Zrl.Order.pending_control_approvals(user)
  ...
  def pending_completion_entries(conn, params) do
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    view_fuel = Zrl.Order.initiated_fuel_entries()
    loco_driver = Accounts.get_loco_driver_data()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
    clients = Accounts.list_tbl_clients()
  ...
  def view_completion_form(conn, params) do
    ...
    view_fuel = Zrl.Order.all_fuel_pending_monitoring(params["id"], "PENDING_COMPLETION", user)
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    loco_driver = Accounts.get_loco_driver_data()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
  ...
  def approver_view_fuel_form(conn, params) do
    user = conn.assigns.user
    batch_items = Order.list_fuel_monitoring_batch_items(params["id"])
    fuel_requisite = Zrl.Order.all_fuel_pending_monitoring(params["id"], "PENDING_CONTROL", user)
    loco_driver = Accounts.get_loco_driver_data()
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
  ...
  def display_requisiste_details(conn, %{"requisition_no" => requisition_no}) do
    user = conn.assigns.user
    batch_items = Zrl.Order.all_fuel_monitoring_entries(requisition_no, user)
  ...
  def back_office_fuel_approval(conn, params) do
    fuel_approval = Zrl.Order.pending_backoffice_approvals()
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    loco_driver = Accounts.get_loco_driver_data()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
  ...
  def back_office_form_details(conn, params) do
    user = conn.assigns.user
    batch_items = Order.list_fuel_monitoring_batch_items(params["id"])
    fuel_approval = Zrl.Order.all_fuel_pending_monitoring(params["id"], "PENDING_APPROVAL", user)
    loco_driver = Accounts.get_loco_driver_data()
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
    ...
    section = SystemUtilities.list_tbl_section()
    train_type = SystemUtilities.list_tbl_train_type()
    user_name = Zrl.Order.get_users_name()
  ...
  def monthly_report(conn, params) do
    ...
    fuel_summary =
      quarter
      |> Zrl.Order.get_fuel_monitor_by_date(year)
      |> format_summary()
    ...
    distance = Zrl.Order.get_fuel_monitor_by_date(quarter, year)
    tons = Zrl.Order.lookup_tonnage(quarter, year)
  ...
  def rejected_requisite_table(conn, params) do
    user = conn.assigns.user
    rejected_requisite = Zrl.Order.get_rejected_requisite(user)
    loco_driver = Accounts.get_loco_driver_data()
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
  ...
  def rejected_requisite_details(conn, params) do
    user = conn.assigns.user
    batch_items = Order.list_fuel_monitoring_batch_items(params["id"])
    rejected_requisite = Zrl.Order.all_fuel_pending_monitoring(params["id"], "REJECTED", user)
    loco_driver = Accounts.get_loco_driver_data()
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
  ...
  def fuel_requisite_table(conn, params) do
    fuel = Zrl.Order.get_complete_fuel_requisite()
    loco_driver = Accounts.get_loco_driver_data()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
    clients = Accounts.list_tbl_clients()
  ...
  def fuel_report_entries(conn, params) do
    user = conn.assigns.user
    batch_items = Order.list_fuel_monitoring_batch_items(params["id"])
    fuel = Zrl.Order.all_fuel_pending_monitoring(params["id"], "COMPLETE", user)
    loco_driver = Accounts.get_loco_driver_data()
    refuel_type = SystemUtilities.list_tbl_refueling_type()
    fuel_rate = SystemUtilities.list_tbl_fuel_rates()
    wagons = SystemUtilities.list_tbl_wagon()
  ...
  def fuel_exco_report(conn, params) do
    # fuel_category = Zrl.Order.get_fuel_monitor_by_date() |> Enum.group_by(&(&1.category))
    company = SystemUtilities.list_company_info()
  ...
  def weekly_fuel_report(conn, params) do
    ...
    fuel_summary =
      month
      |> Zrl.Order.get_fuel_request_weekly(year)
    ...
    distance = Zrl.Order.get_fuel_request_weekly(month, year)

    comltive_dist =
      Enum.reduce(distance, 0, fn entry, sum -> sum + Decimal.to_float(entry.distance) end)

    tons = Zrl.Order.lookup_weekly_tonnage(month, year)

    sect_consumption =
      month
      |> Zrl.Order.get_consumption_by_routes(year)
      |> format_weekly_sec_consumption()

    mvt_exception = Zrl.Order.get_mvt_exceptions(month, year)
  ...
  def weekly_fuel_smry_report_pdf(
    ...
    # user = conn.assigns.user
    entries = Zrl.Order.get_fuel_request_weekly(month, year)
    IO.inspect(month, label: "PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP")
    content = Zrl.Workers.WeeklyFuelSummaryReport.generate(entries, month, year, weeks, params)
  ...
  #   fuel_summary =
  #     quarter
  #     |> Zrl.Order.get_refuel_depo_by_month(year)
  #     |> format_depo_summary()
  ...
  #   fuel_rate = SystemUtilities.get_fuel_rate_by_date(quarter, year)
  #   distance = Zrl.Order.get_refuel_depo_by_month(quarter, year)
  #   tons = Zrl.Order.monthly_tonnage_lookup(quarter, year)
  ...
  def section_summary_generate_pdf(conn, %{"start_dt" => start_dt, "end_dt" => end_dt}) do
    ...
    content = Zrl.Workers.FuelSectionSummary.generate(entries, start_dt, end_dt)
  ...
  def load_depo_report(conn, params),
    do:
      json(conn, %{
        "data" =>
          List.wrap(
            Zrl.SystemUtilities.get_depo_summary(
              params["start_date"],
              params["end_date"],
              conn.assigns.user
            )
  ...
  def depo_summary_generate_pdf(conn, %{"start_dt" => start_dt, "end_dt" => end_dt}) do
    ...
    content = Zrl.Workers.DepoSummary.generate(entries, start_dt, end_dt)
  ...
  def submit_fuel_request(conn, %{"entries" => params, "batch_id" => id}) do
    ...
    |> case do
      {:ok, _} ->
        ...
        Zrl.Order.update_batch(Zrl.Order.get_batch!(id), %{status: "C"})
    ...
    |> Enum.map(fn {item, index} ->
      entry =
        if(to_string(item["id"]) == "",
          ...
          else: Zrl.Order.get_fuel_monitoring!(item["id"])
        )
  ...
  def control_submit_fuel_request(
    ...
    items = Zrl.Order.get_fuel_by_batch_id(requisition_no)
    user = conn.assigns.user
  ...
  defp handle_submit_approval(user, params, status) do
    ...
    |> Enum.map(fn {item, index} ->
      entry =
        if(to_string(item["id"]) == "", do: %FuelMonitoring{maker_id: user.id}, else: Zrl.Order.get_fuel_monitoring!(item["id"]))
  ...
  def report_generator(search_params, source, user) do
    ...
    Zrl.Order.fuel_report_lookup(source, Map.put(search_params, "isearch", ""), user)
  end
  ...
  def fuel_requisite_report_lookup(conn, params) do
    ...
    results = Zrl.Order.fuel_report_lookup(search_params, start, length, conn.assigns.user)
  ...
  def lookup_fuel_rate(conn, %{"station_id" => station_id, "month" => month}) do
    rate = Zrl.SystemUtilities.lookup_fuel_rate(station_id, month)
    json(conn, %{"data" => List.wrap(rate)})
  end

  # def lookup_loco_type(conn, %{"loco_number" => loco_number}) do
  #   type = Zrl.Locomotives.lookup_loco_type(loco_number)
