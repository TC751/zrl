defmodule ZrlWeb.TariffLineController do
  ...
  alias Zrl.{SystemUtilities, Accounts}
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.TariffLineController.authorize/1]
  ...
  def tariff_lookup(conn, %{
    ...
    consignment =
      Zrl.SystemUtilities.tariffline_lookup(
        client_id,
        orign_station,
        destin_station,
        commodity,
  ...
  def tariff_rate_lookup(conn, %{"id" => id}) do
    rate = Zrl.SystemUtilities.tariff_line_rate_lookup(id)
    tariff = Zrl.SystemUtilities.tariff_line_item_look(id)
  ...
  # {:ok, items} = ZrlWeb.TariffLineController.extract_xlsx("C:/Users/Admin/Desktop/book2.xlsx")

  # Enum.each(items, fn x -> ZrlWeb.TariffLineController.upload_excel(x) end)
  ...
  # {:ok, items} = ZrlWeb.TariffLineController.extract_xlsx("D:/wagon.xlsx")

  # Enum.each(items, fn x -> ZrlWeb.TariffLineController.handle_create(x) end)

  # def handle_create(params) do

  #   Ecto.Multi.new()
  #   |> Ecto.Multi.insert(:create,  Zrl.SystemUtilities.Wagon.changeset(%Zrl.SystemUtilities.Wagon{}, params))
  ...
  #   |> Enum.map(fn {item, index} ->
  #     Ecto.Multi.new()
  #     |> Ecto.Multi.insert_or_update({:consignment, index}, Zrl.SystemUtilities.Wagon.changeset(%Zrl.SystemUtilities.Wagon{maker_id: 1}, item))
