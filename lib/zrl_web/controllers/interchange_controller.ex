defmodule ZrlWeb.InterchangeController do
  use ZrlWeb, :controller

  require Logger
  alias Zrl.{Repo, Activity.UserLog, Accounts, SystemUtilities, Tracking, Locomotives}
  ...
  alias Zrl.SystemUtilities.{FileUploadError, Commodity, Wagon, Station}
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.InterchangeController.authorize/1]
       when action not in [
        :unknown,
  ...
  defp check_defects(%{"defects" => _defects} = item, user, index, uuid, wagon_status_id) do
    wag = Zrl.SystemUtilities.get_wagon!(item["wagon_id"])
  ...
  def interchange_batch_entries(conn, params) do
    interchange =
      Zrl.Tracking.interchange_entry_lookup(params["id"], "PENDING_APPROVAL", "OFF_HIRE")

    render(conn, "interchange_approval_entries.html", interchange: interchange)
  end

  def interchange_batch_entries_lookup(conn, params) do
    interchange = Zrl.Tracking.get_interchange_batch_by_uuid(params["id"], params["status"])
    json(conn, %{"data" => interchange})
  end

  def interchange_defect_lookup(conn, %{"id" => id, "admin" => admin_id}) do
    defect =
      Zrl.SystemUtilities.interchange_defect_spare_lookup(id, admin_id)
      |> Enum.reject(&(&1.amount == nil))

    wagon = Zrl.Tracking.interchange_hired_report_list_lookup(id)

    json(conn, %{"data" => defect, "wagon" => wagon})
  end

  def interchange_defect_spare_lookup(conn, %{"id" => id, "admin_id" => admin_id}) do
    defect = Zrl.SystemUtilities.interchange_defect_spares_lookup(id, admin_id)
  ...
  defp handle_close_interchange(user, items) do
    items
    |> Enum.map(fn {index, item} ->
      entry = Zrl.Tracking.get_interchange!(item["id"])
  ...
  defp handle_interchange_batch_off_hire(user, params) do
    ...
    |> Enum.map(fn {index, item} ->
      entry = Zrl.Tracking.get_interchange!(item["id"])
      new_entry = Zrl.Tracking.interchange_hired_report_list_lookup(item["id"])
  ...
  defp handle_single_interchange(user, params, new_defects) do
    entry = Zrl.Tracking.get_interchange!(params["id"])
    item = Zrl.Tracking.interchange_hired_report_list_lookup(params["id"])
    wag = Zrl.SystemUtilities.get_wagon!(item.wagon_id)
  ...
  def interchange_report_lookup(conn, %{"type" => "HAULAGE"} = params) do
    ...
    entries =
      Enum.map(entries(results), fn item ->
        %{item | loco_no: ZrlWeb.MovementView.locomotives_list(item)}
      end)
  ...
  def interchange_batch_report_entries(conn, params) do
    interchange = Zrl.Tracking.interchange_entry_lookup(params["id"], "COMPLETE", "OFF_HIRE")
  ...
  defp report_generator(
    ...
    Zrl.Tracking.interchange_off_hire_report_list_lookup(
      source,
  ...
  defp report_generator(
    ...
    Zrl.Tracking.interchange_hired_report_list_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.material_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.auxiliary_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.auxiliary_daily_summary_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.loco_detention_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.loco_detention_summary_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.haulage_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.mechanical_bills_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Tracking.demurrage_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  defp report_generator(
    ...
    Zrl.Order.works_order_report_lookup(
    source,
    Map.put(search_params, "isearch", ""),
    user
    )
  ...
  defp report_generator(search_params, source, user) do
    Zrl.Tracking.interchange_off_hire_report_lookup(
      source,
      Map.put(search_params, "isearch", ""),
      user
    )
  ...
  def train_no_lookup(conn, %{"train_no" => train_no}) do
    items = Zrl.Order.lookup_train_no(String.trim(train_no))
    json(conn, %{"data" => List.wrap(items)})
  end
  ...
  def modify_wagon_hire(conn, params) do
    entry = Zrl.Tracking.interchange_hired_report_list_lookup(params["id"])
    stations = SystemUtilities.list_tbl_station()
    defects = SystemUtilities.list_tbl_defects("INTL") |> Enum.reject(&(&1.status != "A"))
    condition = SystemUtilities.list_tbl_condition() |> Enum.reject(&(&1.status != "A"))
    admins = Accounts.list_tbl_railway_administrator()
  ...
  defp handle_wagon_tracker(user, params) do
    ...
    |> Enum.map(fn {index, item} ->
      entry = Zrl.Tracking.get_interchange!(item["id"])
      new_entry = Zrl.Tracking.get_int_wagon(item["id"])
  ...
  def haulage_item_lookup(conn, %{"id" => id}) do
    entry = Tracking.haulage_item_lookup(id)
    item = %{entry | loco_no: ZrlWeb.MovementView.locomotives_list(entry)}
    json(conn, %{"data" => item})
  end
  ...
  defp prepare_error_excel(error_changesets, filename) do
    content = ZrlWeb.InterchangeView.gen_error_upload_excel(%{data: error_changesets})
  ...
  defp handle_update_wagon_hire_defects(user, params, defects) do
    entry = Zrl.Tracking.get_interchange!(params["id"])
    wag = Zrl.SystemUtilities.get_wagon!(params["wagon_id"])
