defmodule ZrlWeb.ConsignmentController do
  use ZrlWeb, :controller

  alias Zrl.Consignments
  alias Zrl.Order.{Consignment, Movement}
  alias Zrl.Order
  alias Zrl.Accounts
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.ConsignmentController.authorize/1]
       when action not in [
              :unknown,
  ...
  def gen_new_doc_seq_no() do
    ...
    {:ok, %{columns: _columns, rows: rows}} = Zrl.Repo.query(query, [])
  ...
  def consignment_sales_orders_batch_entries(conn, %{
    ...
    rate =
      case tarriff_id do
        "" -> []
        _ -> Zrl.SystemUtilities.tariffline_lookup(tarriff_id)
      end
  ...
  #     {:ok, _} ->
  #       Zrl.Order.update_batch(Zrl.Order.get_batch!(id), %{status: "C"})
  #       # Zrl.Emails.Email.send_consignment_initized(client_id, sale_order)
  ...
  def verification_consignment(conn, %{"batch" => id, "status" => status}) do
    items = Zrl.Order.get_consignment_batch_items(id)
    ...
    |> case do
      {:ok, _} ->
        case status do
          "REJECTED" ->
            Zrl.Order.update_batch(Zrl.Order.get_batch!(id), %{status: "O"})

            Zrl.Emails.Email.rejected_consignment(id)

          "PENDING_APPROVAL" ->
            Zrl.Order.update_batch(Zrl.Order.get_batch!(id), %{status: "V"})
        end
      ...
      {:ok, _} ->
        case status do
          ...
          "PENDING_INVOICE" ->
            ...
            # Zrl.Emails.Email.send_consignment_approved( consignment_smry["customer_id"], consignment_smry["sale_order"])
        end
  ...
  def pending_consign_lookup(conn, _params) do
    consignment = Zrl.Consignments.pending_consign_lookup()
  ...
  def consignment_report_batch_lookup(conn, params) do
    ...
    results =
      Zrl.Order.consignment_report_lookup(
        search_params,
        start,
        length,
        conn.assigns.user,
  ...
  def consignment_batch_entries(conn, params) do
    batch_items = Zrl.Order.list_consignment_batch_item(params["batch"])
  ...
  def consignment_batch_lookup(conn, %{"status" => status}) do
    user = conn.assigns.user
    empty_commodity = SystemUtilities.empty_commodity_lookup().id
    consignment = Zrl.Order.consignment_batch_lookup(status, user, empty_commodity)
    json(conn, %{"data" => List.wrap(consignment)})
  end
  ...
  def monthly_income_generate_pdf(conn, %{
    ...
    content = Zrl.Workers.ConsignmentMonthlyIncome.generate(entries, start_dt, end_dt, type)
  ...
  def haulage_invoice_generate_pdf(conn, %{"start_dt" => start_dt, "end_dt" => end_dt}) do
    ...
    content = Zrl.Workers.HaulageInvoice.generate(entries, start_dt, end_dt)
  ...
  def unmatched_aging_generate_pdf(conn, %{"start_dt" => start_dt, "end_dt" => end_dt}) do
    ...
    content = Zrl.Workers.UnmatchedAging.generate(entries, start_dt, end_dt)
  ...
  def search_for_consignment(conn, %{"station_code" => station_code, "wagon" => wagon_id}) do
    consignment = Zrl.Order.search_for_consignment_by_station_code(wagon_id, station_code)
    json(conn, %{"data" => consignment})
  end

  # def mvt_search_for_consignment(conn, %{"station_code" => station_code, "wagon" => wagon_id}) do
  #   consignment = Zrl.Order.test_consgsign_search(wagon_id, station_code)
  #   json(conn, %{"data" => consignment})
  # end
  ...
  def lookup_consignment(conn, %{
    ...
    consignment =
      Zrl.Order.search_for_consignment_order_item(
        commodity_id,
        document_date,
        destination_id,
        origin_id,
  ...
  def excel_exp(conn, %{"report_type" => "INVOICING_LIST"}) do
    user = conn.assigns.user
    entries = Zrl.Order.consignment_batch_lookup_excel("PENDING_INVOICE", user)
  ...
  def report_generator(search_params, source, user) do
    ...
    Zrl.Order.consignment_report_excel(
      source,
  ...
  def manual_matching_report_lookup(conn, params) do
    ...
    results =
      Zrl.Order.manual_matching_report_lookup(
        search_params,
        start,
        length,
        unmatched_period,
  ...
  def unmatched_movement_lookup(conn, %{"id" => id}) do
    ...
    movement_list =
      Zrl.Order.get_related_movement_items(
  ...
  def manual_match_entries(conn, %{
    ...
    movenment = Zrl.Order.get_movement!(movenment_id)
  ...
  def search_client_name(conn, %{"search" => search_term, "page" => start}) do
    results = Zrl.Accounts.search_client("%#{search_term}%", String.to_integer(start))
  ...
  def consign_delivery_note(conn, %{"id" => batch_id}) do
    ...
    content = Zrl.Workers.ConsignDeliveryNote.generate(item)
  ...
  def station_code_lookup(conn, %{"station_code" => station_code}) do
    code = Zrl.Order.station_code_lookup(String.trim(station_code))
    json(conn, code)
  end
