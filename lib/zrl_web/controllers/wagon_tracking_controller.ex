defmodule ZrlWeb.WagonTrackingController do
  use ZrlWeb, :controller

  alias Zrl.Tracking
  alias Zrl.Accounts
  alias Zrl.SystemUtilities
  alias Zrl.Tracking.{WagonTracking, WagonTrkSpares}
  alias Zrl.{Repo, Activity.UserLog}
  alias Zrl.SystemUtilities
  alias Zrl.SystemUtilities.{Wagon_defect, Wagon}
  alias Zrl.Repo
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.WagonTrackingController.authorize/1]
       when action not in [:unknown]
  ...
  defp handle_create(user, params) do
    ...
    # wagon_load_status =
    #   case Zrl.Order.search_for_train_list_entry(params["wagon_id"], params["train_no"]) do
    #     nil ->
    #       wag.load_status
  ...
  def view_wagon_position_report(conn, _params) do
    train_num = Zrl.SystemUtilities.list_tbl_train_routes()
  ...
  def delayed_wagons_generate_pdf(conn, %{"start_dt" => start_dt, "end_dt" => end_dt}) do
    ...
    content = Zrl.Workers.DelayedWagons.generate(entries, start_dt, end_dt)
  ...
  def view_wagon_tracker_entries(conn, params) do
    ...
    results = Zrl.Tracking.get_all_wagon_tracker(search_params, start, length, conn.assigns.user)
  ...
  defp process_wagon_tracking_report(conn, source, %{"report_type" => "WAGON_POSITION"} = params) do
    ...
    Zrl.Tracking.get_all_wagon_position(source, Map.put(params, "isearch", ""), user)
    |> Repo.all()
  end
  ...
  defp process_wagon_tracking_report(
    ...
    Zrl.Tracking.wagon_allocation_lookup(source, Map.put(params, "isearch", ""), user)
    |> Repo.all()
  end
  ...
  defp process_wagon_tracking_report(conn, source, %{"report_type" => "WAGON_CONDITION"} = params) do
    ...
    Zrl.Tracking.get_all_wagons_by_condition(source, Map.put(params, "isearch", ""), user)
    |> Repo.all()
  end
  ...
  defp process_wagon_tracking_report(
    ...
    Zrl.Tracking.get_all_wagon_yard_position(source, Map.put(params, "isearch", ""), user)
    |> Repo.all()
  end
  ...
  defp process_wagon_tracking_report(conn, source, params) do
    ...
    Zrl.Tracking.get_all_wagon_tracker(source, Map.put(params, "isearch", ""), user)
    |> Repo.all()
  end
  ...
  def view_wagon_position_entries(conn, params) do
    ...
    results = Zrl.Tracking.get_all_wagon_position(search_params, start, length, conn.assigns.user)
  ...
  def view_wagon_yard_position_entries(conn, params) do
    ...
    results =
      Zrl.Tracking.get_all_wagon_yard_position(search_params, start, length, conn.assigns.user)
  ...
  def view_wagon_delayed_entries(conn, params) do
    ...
    results = Zrl.Tracking.get_all_wagons_delayed(search_params, start, length, conn.assigns.user)
  ...
  def view_wagon_by_condition_entries(conn, params) do
    ...
    results =
      Zrl.Tracking.get_all_wagons_by_condition(search_params, start, length, conn.assigns.user)
  ...
  def view_wagon_allocation_entries(conn, params) do
    ...
    results =
      Zrl.Tracking.wagon_allocation_lookup(search_params, start, length, conn.assigns.user)
  ...
  def update_wagon_mvt_status() do
    items =
      Zrl.Tracking.wagon_days_lookup(Zrl.SystemUtilities.list_company_info().wagon_mvt_status)

    Enum.with_index(items, 1)
    |> Enum.map(fn {item, index} ->
      entry = Zrl.SystemUtilities.get_wagon!(item)
  ...
  def bad_order_average_entries(conn, params) do
    ...
    results = Zrl.Tracking.get_bad_order_average(search_params, start, length, conn.assigns.user)
  ...
  def mvt_train_lookup(conn, %{"train_no" => train_no}) do
    items =
      case WagonTracking.exists?(train_no: train_no) do
        false ->
          Zrl.Order.mvt_lookup_train_no(train_no)
  ...
  def wagon_summary_report(conn, _params) do
    ...
    render(conn, "wagon_summary_report.html",
      all_zrl_wagons: zrl_wagons,
      zrl_loaded_wagons: zrl_loaded_wagons,
      zrl_empty_wagons: zrl_empty_wagons,
      zrl_wagon_fleet: zrl_wagon_fleet,
      zrl_go_wagons: zrl_go_wagons,
      zrl_total_fleet: zrl_total_fleet,
      zrl_total_go_fleet: zrl_total_go_fleet,
      zrl_empty_wagons_by_domain: zrl_empty_wagons_by_domain,
      zrl_loaded_wagons_by_domain: zrl_loaded_wagons_by_domain,
      ...
      zrl_total_non_active: zrl_total_non_active,
      zrl_total_active: zrl_total_active,
  ...
  def generate_wagon_summary_pdf(conn, _params) do
    content = Zrl.Workers.WagonSummary.generate()
