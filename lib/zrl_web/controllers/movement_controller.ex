defmodule ZrlWeb.MovementController do
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.MovementController.authorize/1]
       when action not in [
          :search_station,
          :search_loco_number,
    ...
    |> Enum.map(fn {item, index} ->
      entry =
        if(to_string(item["id"]) == "",
          ...
          else: Zrl.Order.get_movement!(item["id"])
        )
  ...
  def submit_movement(conn, %{"entries" => params, "batch_id" => batch_id, "loco_no" => loco_no}) do
    ...
    |> case do
      {:ok, _} ->
        # Zrl.Order.update_batch(Zrl.Order.get_batch!(batch_id), %{status: "C"})
        json(conn, %{"info" => "sucesss"})
  ...
  defp handle_submit(user, params, loco_no, batch_id) do
    ...
    batch = Zrl.Order.get_batch!(batch_id)
  ...
  def movement_detached_entries(conn, params) do
    ...
    stations = Zrl.SystemUtilities.list_tbl_station()
    wagons = Zrl.SystemUtilities.list_tbl_wagon()
    clients = Zrl.Accounts.list_tbl_clients()
    commodity = Zrl.SystemUtilities.list_tbl_commodity()
    loco = Zrl.Locomotives.list_tbl_locomotive()
  ...
  defp handle_detach_wagon(user, params) do
   ...
   [train | _] = params["entries"] |> Map.values()
    ...
    |> Enum.map(fn {index, item} ->
      ...
      item = Zrl.Order.get_movement!(item["id"])
  ...
  defp handle_attach_detected_wagons(user, params, train) do
    ...
    |> Enum.map(fn {index, item} ->
      ...
      entry = Zrl.Order.get_movement!(item["id"])
  ...
  def movement_pending_approval(conn, _params) do
    user = conn.assigns.user
    batches = Zrl.Order.movement_batch_pending_approval(user)
  ...
  def approve_movement(conn, %{"batch_id" => id, "status" => status}) do
    ...
    |> case do
      {:ok, _} ->
        Zrl.Order.update_batch(Zrl.Order.get_batch!(id), %{status: "COMPT"})
        json(conn, %{"info" => "sucesss"})
  ...
  def reject_movement(conn, %{"batch_id" => id}) do
    ...
    |> case do
      {:ok, _} ->
        Zrl.Order.update_batch(Zrl.Order.get_batch!(id), %{status: "R"})
        Zrl.Emails.Email.rejected_movement(id)
        json(conn, %{"info" => "sucesss"})
  ...
  def movement_draft_entries(conn, params) do
    batch_items = Order.list_movement_batch_items(params["batch_id"])
    batch = Order.get_batch!(params["batch_id"])
    stations = Zrl.SystemUtilities.list_tbl_station() |> Enum.reject(&(&1.status != "A"))
    wagons = Zrl.SystemUtilities.list_tbl_wagon() |> Enum.reject(&(&1.status != "A"))
    clients = Zrl.Accounts.list_tbl_clients() |> Enum.reject(&(&1.status != "A"))
    commodity = Zrl.SystemUtilities.list_tbl_commodity() |> Enum.reject(&(&1.status != "A"))
    loco = Zrl.Locomotives.list_tbl_locomotive() |> Enum.reject(&(&1.status != "A"))
  ...
  def movement_report_lookup(conn, params) do
    ...
    results = Zrl.Order.movement_report_lookup(search_params, start, length, conn.assigns.user)
  ...
  def report_generator(search_params, source, user) do
    ...
    Zrl.Order.movement_report_lookup_excel(
      source,
  ...
  def movement_without_congnmt_batch(conn, params) do
    ...
    #   end
    stations = Zrl.SystemUtilities.list_tbl_station() |> Enum.reject(&(&1.status != "A")) |> Enum.sort_by(&(&1.description), :asc)
    clients = Zrl.Accounts.list_tbl_clients() |> Enum.reject(&(&1.status != "A")) |> Enum.sort_by(&(&1.client_name), :asc)
    commodity = Zrl.SystemUtilities.list_tbl_commodity() |> Enum.reject(&(&1.status != "A")) |> Enum.sort_by(&(&1.description), :asc)
    loco = Zrl.Locomotives.list_tbl_locomotive() |> Enum.reject(&(&1.status != "A"))
  ...
  def update_movement_item(conn, params) do
    item = Zrl.Order.get_movement!(params["id"])
  ...
  def lookup_stn_owner(conn, %{"movement_reporting_station_id" => movement_reporting_station_id}) do
    owner = Zrl.SystemUtilities.station_owner_lookup(movement_reporting_station_id)
    json(conn, %{"data" => List.wrap(owner)})
  end

  def invoice_lookup(conn, %{"station_code" => station_code, "wagon_id" => wagon_id}) do
    invoice = Zrl.Order.invoice_lookup(station_code, wagon_id)
  ...
  def  works_order_pdf(conn, %{"id" => id}) do
    item = Order.works_order_lookup(id)
    content = Zrl.Workers.WorksOrder.generate(item)
