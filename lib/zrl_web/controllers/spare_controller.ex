defmodule ZrlWeb.SpareController do
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.SpareController.authorize/1]
       when action not in [:unknown]
  ...
  # {:ok, items} = ZrlWeb.SpareController.extract_xlsx("C:/Users/Admin/OneDrive/Desktop/spares.xlsx")

  # Enum.each(items, fn x -> ZrlWeb.SpareController.upload_excel(x) end)

  # {:ok, items} = ZrlWeb.SpareController.extract_xlsx("D:/wagon.xlsx")

  # Enum.each(items, fn x -> ZrlWeb.SpareController.handle_create(x) end)

  # def handle_create(params) do

  #   Ecto.Multi.new()
  #   |> Ecto.Multi.insert(:create,  Zrl.SystemUtilities.Spare.changeset(%Zrl.SystemUtilities.Spare{}, params))
  #   |> Ecto.Multi.run(:user_log, fn repo, %{create: create} ->
