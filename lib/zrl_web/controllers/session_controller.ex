defmodule ZrlWeb.SessionController do
        ...
        with {:error, _reason} <- Auth.confirm_password(user, String.trim(params["password"])) do
          ...
          {:ok, _} ->
            cond do
              user.status == "A" ->
                {:ok, _} =
                  Zrl.Activity.create_user_log(%{user_id: user.id, activity: "logged in"})
  ...
  def signout(conn, _params) do
    {:ok, _} =
      Zrl.Activity.create_user_log(%{user_id: conn.assigns.user.id, activity: "logged out"})
  ...
  defp prepare_login_attempt(user) do
    max_attempts = Zrl.SystemUtilities.list_company_info().login_attempts
