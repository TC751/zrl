defmodule ZrlWeb.LocomotiveController do
  use ZrlWeb, :controller

  alias Zrl.{Locomotives, SystemUtilities}
  alias Zrl.Locomotives.Locomotive
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.LocomotiveController.authorize/1]
       when action not in [:unknown]

  def index(conn, _params) do
    owners = Zrl.Accounts.list_tbl_railway_administrator() |> Enum.reject(&(&1.status != "A"))
  ...
  def update(conn, params) do
    item = Zrl.Locomotives.get_locomotive!(params["id"])
  ...
  def delete(conn, %{"id" => id}) do
    Zrl.Locomotives.get_locomotive!(id)
  ...
  def change_status(conn, %{"id" => id} = params) do
    loco = Zrl.Locomotives.get_locomotive!(id)
