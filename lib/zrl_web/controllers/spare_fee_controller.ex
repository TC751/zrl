defmodule ZrlWeb.SpareFeeController do
  use ZrlWeb, :controller

  alias Zrl.SystemUtilities
  alias Zrl.SystemUtilities.SpareFee
  alias Zrl.Logs.UserLog
  alias Zrl.{Repo, Activity.UserLog, Accounts}
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.SpareFeeController.authorize/1]
  ...
  def admin_defect_spare_lookup(conn, %{
    ...
    rates = Zrl.SystemUtilities.defect_spare_lookup(defect_id, admin_id, date)
    json(conn, %{"data" => rates})
  end

  def admin_defect_spare_lookup(conn, params) do
    rates = Zrl.SystemUtilities.defect_spare_lookup(params["defect_id"], params["admin_id"])
    json(conn, %{"data" => rates})
  end
  ...
  # {:ok, items} = ZrlWeb.SpareFeeController.extract_xlsx("C:/Users/Admin/OneDrive/Desktop/tzrsparefees.xlsx")

  # Enum.each(items, fn x -> ZrlWeb.SpareFeeController.upload_excel(x) end)

  # {:ok, items} = ZrlWeb.SpareFeeController.extract_xlsx("D:/wagon.xlsx")

  # Enum.each(items, fn x -> ZrlWeb.SpareFeeController.handle_create(x) end)

  # def handle_create(params) do

  #   Ecto.Multi.new()
  #   |> Ecto.Multi.insert(:create,  Zrl.SystemUtilities.SpareFee.changeset(%Zrl.SystemUtilities.SpareFee{}, params))
  #   |> Ecto.Multi.run(:user_log, fn repo, %{create: create} ->
