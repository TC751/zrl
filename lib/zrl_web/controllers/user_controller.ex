defmodule ZrlWeb.UserController do
  use ZrlWeb, :controller
  import Ecto.Query, warn: false

  alias Zrl.{Accounts.User, Repo}
  alias Zrl.{Repo, Activity.UserLog}
  alias Zrl.Accounts
  alias ZrlWeb.Auth
  require Logger
  alias Zrl.{Accounts.User, Auth, Accounts, Repo}
  alias Zrl.Emails.Email
  require Logger
  ...
  plug(
    ZrlWeb.Plugs.Authenticate,
    [module_callback: &ZrlWeb.UserController.authorize/1]
    when action not in [
           :unknown,
           :new_password,
           :perform_reset,
  ...
  def dashboard(conn, _params) do
    {stats, totals} =
      (Zrl.Order.consignment_dashboard_params(conn.assigns.user) ++
         Zrl.Order.movement_dashboard_params(conn.assigns.user) ++
         Zrl.Order.fuel_requisite_dashboard_params(conn.assigns.user))
      |> Enum.sort_by(& &1.day)
      |> prepare_dash_result()
  ...
  def total_loaded_wagons() do
    wagons = Zrl.SystemUtilities.total_loaded_wagons_lookup()
  ...
  def perform_reset(conn, user_params) do
    ...
    expiry_days = Zrl.SystemUtilities.list_company_info().password_expiry_days
  ...
  def user_logs(conn, %{"id" => user_id}) do
    with :error <- confirm_token(conn, user_id) do
      ...
      {:ok, user} ->
        user_logs = Zrl.Activity.get_logs_by(user.id)
