defmodule ZrlWeb.DefectController do
  ...
  plug ZrlWeb.Plugs.Authenticate,
       [module_callback: &ZrlWeb.DefectController.authorize/1]
  ...
  def defects_lookup(conn, %{"tracker_id" => tracker_id, "wagon_id" => wagon_id}) do
    ...
    spares = Zrl.Tracking.defect_spares_lookup(tracker_id, wagon_id)
    data = Zrl.SystemUtilities.get_defects_by_ids(ids)

    json(conn, %{"data" => List.wrap(data), "spares" => List.wrap(spares)})
  end

  def defect_spare_lookup(conn, %{"id" => id}) do
    spares = Zrl.SystemUtilities.defect_spare_lookup(id)
